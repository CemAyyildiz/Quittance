...
Soroban helper crate for the Quittance `invoice_created` event.

This crate exists so that every Quittance Soroban contract emits an
`invoice_created` signal with the **same topic layout** and the same
**data shape**. Off-chain indexers, dashboards, and notification
services can then subscribe once and consume the event across
...
`created_at` is the **host-ledger timestamp in seconds** (the standard
...
- Event-name symbol: `"invoice_created"` (15 bytes — fits the small-symbol limit).
- Topic order: `name, invoice_id, seller, payer`. Do not reorder.
- Data tuple order: `amount, asset, created_at`. Do not reorder.

## Public API

```rust
use soroban_sdk::{Address, Env, String};
use event_invoice_created::{publish, topic, topics, data, EVENT_NAME};
...
// Data payload as a single Val encoding (amount, asset, created_at)
...
```bash
cargo test -p event_invoice_created
```
...
* No `env.events().all()` round-trip — `Events::all` is
  ...
  so the acceptance criterion ("Topic builder covered by unit tests")
  is met by topic-and-data-builder tests alone.
