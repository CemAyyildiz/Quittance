...
import { memoValidate } from './memoValidate';
...
describe('memoValidate', () => {
  ...
  it('rejects memos longer than 28 bytes (ASCII)', () => {
    ...
    expect(memoValidate(memo)).toEqual({ valid: false, error: 'Memo exceeds maximum byte length of 28 bytes.' });
  });
  ...
  it('correctly calculates mixed ASCII and Unicode lengths', () => {
    ...
    expect(memoValidate(badMemo)).toEqual({ valid: false, error: 'Memo exceeds maximum byte length of 28 bytes.' });
  });
});
